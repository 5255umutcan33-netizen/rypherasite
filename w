...
<body class="antialiased">
    ...
    <div id="main-content">
        <nav class="p-6 relative flex items-center justify-between max-w-7xl mx-auto w-full">
            <div class="flex-1 flex justify-start">
                <div class="cursor-pointer flex items-center gap-3 transition hover:scale-105" onclick="changePage('home')">
                    <img src="logo.png" alt="Logo" class="h-10 md:h-12 w-auto object-contain rounded-lg shadow-[0_0_15px_rgba(255,255,255,0.1)]" onerror="this.src='https://via.placeholder.com/50x50/1a1a1a/ffffff?text=R'">
            ...
            <div class="flex-1 flex justify-center space-x-8 hidden md:flex text-sm uppercase tracking-widest text-gray-400 font-medium items-center">
                ...
                <a href="#" onclick="changePage('admin')" class="nav-link hover:text-white transition text-red-500" id="nav-admin">Admin</a>
            </div>
            <div class="flex-1 flex justify-end">
                <button id="discord-login-btn" onclick="loginWithDiscord()" class="bg-[#5865F2] hover:bg-[#4752C4] text-white px-6 py-2 rounded-lg font-bold transition-all flex items-center gap-2 shadow-[0_0_15px_rgba(88,101,242,0.4)]">
                    <i class="fa-brands fa-discord text-xl" id="btn-icon"></i><span id="login-text">Giriş Yap</span>
                </button>
            </div>
        </nav>
        ...
        <div id="home-view" class="page-view active">
            <header class="py-24 px-6 text-center mt-10">
                ...
                <p class="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto mb-12 font-light">Hızlı, güvenli ve tamamen optimize edilmiş altyapı.</p>
                <div class="flex flex-col md:flex-row justify-center gap-6">
                    <a href="https://discord.gg/DAVET-LINKI" target="_blank" class="bg-white text-black px-10 py-4 rounded-full font-bold hover:bg-gray-300 transition-all transform hover:scale-105">ŞİMDİ KATIL</a>
        ...
        <div id="scripts-view" class="page-view text-center py-20">
             <div class="glass max-w-2xl mx-auto p-12 rounded-3xl border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                 ...
                 <h3 class="text-2xl font-bold mb-4">Şu An Script Bulunmuyor</h3>
                 <p class="text-gray-400">Sistemimizde şu anda yayında olan bir script bulunmuyor. Çok yakında yeni nesil sistemlerimizle buradayız.</p>
             </div>
        </div>
            ...
            <section class="max-w-3xl mx-auto px-6 py-10">
                <div class="glass p-8 md:p-12 rounded-3xl border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                    <div id="apply-warning" class="text-center py-10">
                        ...
                        <h3 class="text-xl font-bold mb-2">Giriş Yapmanız Gerekiyor</h3>
                    ...
                    <form id="applicationForm" class="space-y-6 hidden" onsubmit="sendApplication(event)">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-bold text-gray-400 mb-2">Başvuru Bölgesi</label>
                                <select id="appRegion" required>
                                    <option value="TR">Türkçe (Türkiye)</option>
                                    <option value="EN">English (Global)</option>
                                </select>
                            </div>
                            <div><label class="block text-sm font-bold text-gray-400 mb-2">İsminiz?</label><input type="text" id="appName" required></div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div><label class="block text-sm font-bold text-gray-400 mb-2">Yaş?</label><input type="number" id="appAge" required></div>
                            <div><label class="block text-sm font-bold text-gray-400 mb-2">Aktiflik?</label><input type="text" id="appActive" required></div>
                        </div>
                        <div><label class="block text-sm font-bold text-gray-400 mb-2">Komut Bilginiz?</label><textarea id="appCmd" rows="3" required></textarea></div>
                        <div><label class="block text-sm font-bold text-gray-400 mb-2">Neden?</label><textarea id="appWhy" rows="4" required></textarea></div>
                        <button type="submit" id="submitBtn" class="w-full bg-[#5865F2] hover:bg-[#4752C4] text-white font-bold py-4 rounded-lg transition-all">
                            <i class="fa-solid fa-paper-plane mr-2"></i> Başvuruyu Gönder
                        </button>
                    </form>
                </div>
            </section>
        ...
        <div id="admin-view" class="page-view text-center py-20">
            <div id="admin-warning" class="glass max-w-2xl mx-auto p-12 rounded-3xl">
                <i class="fa-solid fa-ban text-6xl mb-6 text-red-600"></i>
                <h3 class="text-2xl font-bold text-white mb-4">Yetkisiz Erişim</h3>
                <p class="text-gray-400">Yönetici hesabınızla giriş yapmalısınız.</p>
            </div>
            <div id="admin-dashboard" class="hidden glass p-8 rounded-3xl max-w-2xl mx-auto border border-red-500/30 text-left">
                ...
                <p class="text-gray-400 mt-2">Başvurular başarıyla Discord sunucusuna iletiliyor.</p>
            </div>
        </div>
    ...
    <script>
        const CLIENT_ID = "1491071700715048970";
        const REDIRECT_URI = encodeURIComponent("https://rypherasite-plfwd8xo2-5255umutcan33-netizens-projects.vercel.app/");
        const SAHIP_ID = "345821033414262794";
        
        // WEBHOOK URL'LERI (Senin ilettiğin orijinal URL'ler)
        const WEBHOOK_TR = "https://discord.com/api/webhooks/1491124164042424332/vRWiYUmtb3pF34escp5ygnMnyfrH3bgyCZaKHfmgTDXQE2dxUi7cL-3t0tLN3c_9wDaT";
        const WEBHOOK_EN = "https://discord.com/api/webhooks/1491124036359557362/2v_uo05PuQS1dPSIvdKCRCHcRAaZPS8s8d6G0koGce9fDG4J6BDWAn20cXNnfQoWQgaA";

        let loggedInUser = null;

        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => { document.getElementById('main-content').style.opacity = '1'; document.body.style.overflow = 'auto'; }, 3500);
            setTimeout(() => { document.getElementById('pre-intro').style.display = 'none'; }, 4500);
            checkDiscordLogin();
        });
        ...
        function checkDiscordLogin() {
            ...
            if (accessToken) {
                ...
                .then(res => res.json()).then(user => {
                    loggedInUser = user;
                    document.getElementById('login-text').innerText = " " + user.username;
        ...
        function sendApplication(e) {
            ...
            const payload = {
                ...
                embeds: [{
                    ...
                    fields: [
                        ...
                        { name: "Aktiflik Süresi", value: document.getElementById('appActive').value, inline: false },
                        { name: "Komut Bilgisi", value: document.getElementById('appCmd').value, inline: false },
                        { name: "Neden Yetkili Olmak İstiyor?", value: document.getElementById('appWhy').value, inline: false }
                    ],
            ...
            fetch(targetWebhook, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
            .then(res => { 
                if(res.ok) { 
        ...
        function changePage(pageId) {
            ...
            document.querySelectorAll('.nav-link').forEach(link => { link.classList.remove('active-nav', 'text-white'); link.classList.add('text-gray-400'); });
